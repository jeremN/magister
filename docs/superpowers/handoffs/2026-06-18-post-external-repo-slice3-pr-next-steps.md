...
- **MANUAL PROOF NOT YET RUN.** The spec/plan's end-to-end proof against a REAL throwaway GitHub repo (`gh` is authed as `jeremN`) was intentionally NOT executed by the agent (it would open a real PR). Run it before relying on the slice in anger: `cm run flows/external-repo.yaml --repo <throwaway-clone> --base HEAD` → `cm get` (succeeded) → `cm push <run>` → `cm pr <run>` (opens PR; head `magister/<run>`, generated body) → `cm pr <run>` again (409 with the existing PR URL).
